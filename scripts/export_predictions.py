...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to ground truth directory/file (required for spiritualist, hiertext, and hnla2013)",
    )
    parser.add_argument(
        "--image-ext",
        type=str,
    ...
    elif args.dataset_name == "hnla2013":
        if not args.groundtruth:
            logger.error("--groundtruth is required for the hnla2013 dataset")
            sys.exit(1)
        groundtruth_path = Path(args.groundtruth)
        if not groundtruth_path.exists():
            logger.error(f"Ground truth path does not exist: {groundtruth_path}")
