#!/usr/bin/env python3
"""
Benchmark all models on the HierText dataset.

This script evaluates multiple models (DocLayout-YOLO, DoclingLayoutHeron,
PP-DocLayout-L) on a local HierText split using line-level ground truth grouped
by paragraph (ssu_id) and computes comparative COTe metrics.

Images are expected as ``<image_id>.jpg`` in --dataset; --groundtruth points at
the HierText GT JSON file (e.g. validation.jsonl).
"""
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark all document layout models on HierText")
    parser.add_argument(
        "--dataset",
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="results/hiertext",
        help="Path to output directory (default: results/hiertext)",
    )
    parser.add_argument(
        "--models",
        nargs="+",
    ...
    print("\n" + "=" * total_w)
    print("COMPARATIVE SUMMARY — HIERTEXT")
    print("=" * total_w)
