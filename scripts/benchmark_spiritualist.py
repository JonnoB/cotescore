#!/usr/bin/env python3
"""
Benchmark all models on the Spiritualist dataset.

This script evaluates multiple models (DocLayout-YOLO, DoclingLayoutHeron,
PP-DocLayout-L) on the Spiritualist evaluation set using SSU-tagged ALTO XML
ground truth and computes comparative COTe metrics.

The ALTO XML files in the ground truth directory are expected to have SSU
attributes already assigned to TextLine elements (e.g. via ALTOSSUTagger).
"""
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark all document layout models on the Spiritualist dataset")
    parser.add_argument(
        "--dataset",
        type=str,
        required=True,
        help="Path to directory containing Spiritualist JPEG images",
    )
    parser.add_argument(
        "--groundtruth",
        type=str,
        required=True,
        help="Path to directory containing SSU-tagged ALTO XML files",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="results/spiritualist",
        help="Path to output directory (default: results/spiritualist)",
    )
    parser.add_argument(
        "--models",
        nargs="+",
        ...
        help="Models to benchmark (default: yolo heron ppdoc-l)",
    )
    ...
    print("\n" + "=" * total_w)
    print("COMPARATIVE SUMMARY — SPIRITUALIST")
    print("=" * total_w)
